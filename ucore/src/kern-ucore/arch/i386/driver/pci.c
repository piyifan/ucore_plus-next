...
static int pci_net_ether_attach(struct pci_func *pcif) {    
    kprintf("PCI: %02x:%02x.%d: Network (ethernet) %04x:%04x\n",
...
static int pci_bridge_attach(struct pci_func *pcif) {
    kprintf("PCI: %02x:%02x.%d: Bridge %04x:%04x\n",
            pcif->bus->busno, pcif->dev, pcif->func,
            PCI_VENDOR(pcif->dev_id), PCI_PRODUCT(pcif->dev_id));
    return 0;
}
...
static int pci_display_attach(struct pci_func *pcif) {
    kprintf("PCI: %02x:%02x.%d: Display %04x:%04x\n",
        pcif->bus->busno, pcif->dev, pcif->func,
        PCI_VENDOR(pcif->dev_id), PCI_PRODUCT(pcif->dev_id));
    return 0;
}
...
void pci_func_enable(struct pci_func *f) {
    ...
    if (!(PciCommandWord & PCI_COMMAND_IO)) {
        kprintf("Error: Can not enable IO access.\n");
    } else if (!(PciCommandWord & PCI_COMMAND_MASTER)) {
        kprintf("Error: Can not enable Bus Mastering.\n");
    } else {
        kprintf("enable IO access succeeded\n");
    }
...
int pci_init(void) {
    ...
    kprintf(" pci init finished, %d devices in total\n", totaldev);
    return totaldev;
}
