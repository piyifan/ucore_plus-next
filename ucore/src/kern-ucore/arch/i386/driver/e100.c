...
int ether_e100_attach(struct pci_func *pcif) {
    ...
    if (!dev->cubase)
    {
        kprintf("   Failed to allocate CU ring\n");
        dev_count--;
        return -1;
    }
#ifdef E100_DEBUG
    ...
    if (!dev->rubase)
    {
        kprintf("   Failed to allocate RU ring\n");
        dev_count--;
        return -1;
    }
#ifdef E100_DEBUG
...
scb_status_word e100_print_state(e100_dev *dev) {
    ...
    if (stat.cx) kprintf("   CU executed\n");
    if (stat.fr)
        kprintf("   RU received\n");
    if (stat.cna)
        kprintf("   CU state change\n");
    if (stat.rnr)
        kprintf("   RU not ready\n");
    if (stat.mdi)
        kprintf("   MDI operation completed\n");
    if (stat.swi)
        kprintf("   Software Interrupt\n");
    if (stat.fcp)
        kprintf("   Flow Control Pause\n");
...
void e100_intr(struct trapframe *tf) {
    ...
    if (index == -1)
    {
        kprintf("e100_intr: can't find corresponding device\n");
        return;
    }
    e100_dev *dev = &e100_devs[index];
#ifdef E100_DEBUG_INTR
...
/*        if (dev->cu_next_pkt != 0)
        {
            kprintf("   finished 0x%08x\n", dev->cu_next_pkt);
            if (dev->cu_next_pkt->link != 0)
            {
                kprintf("   starting next packet: "
...
void ring_printinfo(e100_dev *dev) {
    kprintf("Ring Info:   ");
    kprintf("head: 0x%08x  ", dev->cu_next_pkt);
    kprintf("end:  0x%08x\n", dev->cu_last_pkt);
}
...
void ring_init(e100_dev *dev) {
    ...
    for (i=0; i<dev->cucount ; i++)
    {
        ...
        kprintf("CB %d: start 0x%08x next 0x%08x\n", i, cmd, cmd->link);
#endif
    }
...
#ifdef E100_DEBUG_RING
        kprintf("RFD %d: start 0x%08x next 0x%08x\n", i, rfd, rfd->head.link);
#endif
    }
    dev->ru_first = 0;
...
void e100_ru_start(e100_dev *dev) {
#ifdef E100_DEBUG
    kprintf("e100_ru_start addr: %08x\n", PADDR(dev->rubase));
#endif
    dev->ru_full = 0;
...
int e100_receive_dev(e100_dev *dev, void *buffer, int len) {
    int count;
#ifdef E100_DEBUG
    kprintf("e100_receive_dev\n");
#endif
    down(&dev->rulock);
    while (dev->ru_first == 0)
        schedule();
#ifdef E100_DEBUG
    kprintf("ru_first: %08x\n", dev->ru_first);
#endif
...
#ifdef E100_DEBUG
    kprintf("received %d bytes into 0x%08x\n", dev->ru_first->count, buffer);
#endif
...
//      schedule();
        while (dev->ru_last->eof)
        {
            ...
            ethernetif_input(&dev->netif);
#ifdef E100_DEBUG
            kprintf("advance ru_last: from %08x to %08x\n", dev->ru_last, KADDR(dev->ru_last->head.link));
#endif
