...
sys_sem_t sys_sem_new(u8_t count)
{
	...
	kprintf("[%d] sem_new count %d return: %d\n", pls_current->pid, count, sem);
#endif
	return sem;
}
...
#ifdef LWIP_ARCH_DEBUG
	kprintf("[%d] sem: %d ret: %d timeout: %d\n", pls_current->pid, sem, ret, timeout);
#endif
	if (ret == 0)
		return 0;
	else
...
#ifdef LWIP_ARCH_DEBUG
	kprintf("[%d] sem_new count %d return %08x\n", pls_current->pid, count, sem);
#endif
	return sem;
}
...
{
#ifdef LWIP_ARCH_DEBUG
	kprintf("[%d] sem_up %08x\n", pls_current->pid, sem);
#endif

#ifdef USE_KERNEL_SEM
	up(sem);
#else
    usem_up(sem);
#endif
...
{
#ifdef LWIP_ARCH_DEBUG
	kprintf("[%d] sem_down %08x timeout %d\n", pls_current->pid, sem, timeout);
#endif
	if (timeout == 0)
	{
#ifdef USE_USER_SEM
...
#ifdef USE_USER_SEM
	    // U11 : use timeout
	    timeout /= 10; // time unit tranfer
        if (usem_down(sem, timeout) == -E_TIMEOUT)
            return SYS_ARCH_TIMEOUT;
        return 0;
#else
		timeout = timeout / 10;
...
sys_mbox_t sys_mbox_new(int size)
{
	if (size > _MBOX_SIZE)
		return NULL;
#ifdef LWIP_ARCH_DEBUG
	kprintf("[%d] mbox_new size: %d\n", pls_current->pid, size);
#endif
...
{
	int ret = sys_arch_sem_wait(mbox->used, 10);
	if (ret == SYS_ARCH_TIMEOUT)
...
{
	assert(pls_current);
	if (pls_current->lwip_timeouts == NULL) {
		pls_current->lwip_timeouts = (struct sys_timeouts *)kmalloc(sizeof(struct sys_timeouts));
        pls_current->lwip_timeouts->next = NULL;
    }
	return pls_current->lwip_timeouts;
}
